...
def greedy_match(
    ...
    Previously used greedy matching which could produce suboptimal assignments.
    """
    if len(gt) == 0:
        return 0, len(pred), 0, []
    ...
    # Compute pairwise distance matrix: (N_gt, N_pred)
    dist_matrix = np.sqrt(
        np.sum((gt[:, np.newaxis, :] - pred_xy[np.newaxis, :, :]) ** 2, axis=2)
    )

    # Hungarian algorithm on cost matrix (set beyond-threshold pairs to large value)
    large_val = max_dist * 100
    cost = dist_matrix.copy()
    cost[cost > max_dist] = large_val
